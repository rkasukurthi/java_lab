...
public class Solution {
  ...
  private int findMinHealth(int i, int j, int minHealth, int[][] dungenon) {
    ...
    if (dungenon[i][j] >= minHealth) {
      minHealth = 1;
    } else {
      minHealth = minHealth - dungenon[i][j];
    }
    
    if (cache[i][j] > 0) {
      cache[i][j] = Math.min(cache[i][j], minHealth);
    } else {
      cache[i][j] = minHealth;
    }
    if (i == 0) {
    ...
    return Math.min(findMinHealth(i - 1, j, cache[i][j], dungenon),
        findMinHealth(i, j - 1, cache[i][j], dungenon));
  }
