...
public class Solution {
    ...
    /*   *//** SubSequence **/
    /*
     * String lpsequence(String s, int i, int j) { int key = i * 31 + j; if
     * (map.get(key) != null) return map.get(key); String str; if (i == j) {
     * map.put(key, s.substring(i, i + 1)); } else if (s.charAt(i) ==
     * s.charAt(j) && i + 1 == j) { map.put(key, s.substring(i, j + 1)); } else
     * if (s.charAt(i) == s.charAt(j)) { map.put(key, s.substring(i, i + 1) +
     * lps(s, i + 1, j - 1) + s.substring(j, j + 1)); } else { String s1 =
     * lps(s, i, j - 1); String s2 = lps(s, i + 1, j); if (s1.length() >
     * s2.length()) { map.put(key, s1); } else { map.put(key, s2); } } return
     * map.get(key);
     * 
     * }
     */
    ...
    @Test
    public void test5() {
	assertEquals("aaaaaa", longestPalindrome("aaaaaa"));
    }
