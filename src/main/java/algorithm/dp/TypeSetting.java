 ...
 */
  ...
  public int calculatePenalty(String[] lines, int length) {
    int penalty = 0;
    for (int i = 0; i < lines.length; i++) {
      penalty += calculatePenalty(lines[i], length);
    }
    return penalty;
  }

  public int calculatePenalty(String line, int length) {
    return (int) Math.pow(length - line.length(), 2);
  }
   ...
   * Print input text.
   *  
   * @param a, contains indexs to create a new line.
   */
