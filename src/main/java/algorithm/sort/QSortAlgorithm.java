 ...
 * http://java.sun.com/copy_trademarks.html for further important copyright and trademark
 * information and to http://java.sun.com/licensing.html for further important licensing information
 * for the Java (tm) Technology.
 * 
 ...
 * the pivot element was not unique. -Jason Harrison
 * 
 * 21 Jun 1996: Modified code based on comments from Paul Haeberli, and Peter Schweizer
 * (Peter.Schweizer@mni.fh-giessen.de). Used Daeron Meyer's (daeron@geom.umn.edu) code for the new
 * pivoting code. - Jason Harrison
 * 
...
class QSortAlgorithm extends SortAlgorithm {
  ...
  void sort(int a[], int lo0, int hi0) throws Exception {
    ...
    // high cursor is next to low cursor, sort two elements by swapping if necessary
    if (lo == hi - 1) {
      if (a[lo] > a[hi]) {
        int T = a[lo];
        a[lo] = a[hi];
    ...
    /*
     * Recursive calls, elements a[lo0] to a[lo-1] are less than or equal to pivot, elements a[hi+1]
